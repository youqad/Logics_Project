...
class CNF():
            ...
            with open(args[0]) as f:
                for line in f:
                    if line[0] == 'c' or line[0] == 'p':
                        continue
                    line = map(lambda x: int(x) if not isinstance(x, int) else 0, line.split())
                    self.clauses.append(set(takewhile(lambda x: x !=0, line)))
    ...
    def DPLL(self):
        return DPLL(deepcopy(self.clauses), set())
